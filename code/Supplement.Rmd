...
get_binary_points <- dataFull2 %>%
  ...
  scale_y_continuous(name = expression("OD"["450"]), expand = c(0,0)) +
...
plot_points <- plot_grid(get_binary_points, fig1a_original, labels = c("A", "B"), rel_widths = c(1,1.3))
...
fig1a <- dataFull2 %>%
  ...
  ggtitle("Frequency", "Binary") +
    theme(axis.text = element_text(size = 12, color = "black"),
        axis.title = element_text(size = 12, color = "black"),
        axis.line = element_blank(),
        axis.ticks = element_line(color = "black"),
        plot.title = element_text(colour = "black", size = 13.5, face = "bold", hjust = 0.1),
        plot.title.position = "plot",
...
## arrange frequency 
```{r}
fig1 <- plot_grid(fig1a, fig1b, nrow = 1, rel_widths = c(1,1.4), labels = c("C", "D"))
...
## Save
```{r}
FigureS1 <- plot_grid(plot_points, fig1, nrow = 2, rel_heights = c(1,1))

ggsave("../Supplement/FigS1.jpg", FigureS1, width = 9, height = 10)
...
profiling <- plot_grid(get_full_with_conf_binary, get_full_with_conf_variation, nrow = 2, labels = c("A", "B"), rel_heights = c(1,2))

ggsave("../Supplement/Profiling.jpg",profiling, width = 14, height = 8)
...
plot_violin_binary <- total_count %>%
  ...
  scale_fill_manual("sHCoV",values = mycolors) +
  ggtitle("Lifetime seroconversions") +
    theme(axis.text = element_text(size = 12, color = "black"),
        axis.title = element_text(size = 12, color = "black"),
        axis.line = element_blank(),
        axis.ticks = element_line(color = "black"),
...
### Arrange
```{r}
get_stoch_traj <- plot_grid(fig4a,NULL,fig4a2, rel_widths = c(1,0.3,2.65), labels = c("B", "", "C"), nrow = 1)
...
plot_full_stoch <- plot_grid(plot_violin_binary, get_stoch_traj, fig4b, nrow = 3, rel_heights = c(0.8,0.8,0.7), labels = c("A","", "D"))


ggsave("../Supplement/Stochastic_Figure.jpg", plot_full_stoch, width = 12, height = 10)
```
...
get_binary_fig4 <- plot_grid(fig41, fig2d, nrow = 2, rel_heights = c(1,0.8), labels = c("A", "B"))


ggsave("../Supplement/Binaryfig4.jpg", get_binary_fig4, width = 8, height = 8)
```
...
fig_rmse_direct <- return_simulations %>%
  filter(exposure != "Binary", waning == "Direct") %>%
  rename(exposure_hypothesis = exposure) %>%
  select(time, Prop, Type, Group, waning, choose_rho, exposure_hypothesis, strain = strain_type) %>%
  rename(Serostatus = Group) %>%
...
fig_rmse_sequential <- return_simulations %>%
  filter(exposure != "Binary", waning == "Laddered") %>%
  rename(exposure_hypothesis = exposure) %>%
  select(time, Prop, Type, Group, waning, choose_rho, exposure_hypothesis, strain = strain_type) %>%
  rename(Serostatus = Group) %>%
...
fig3bc <- plot_grid(NULL, fig_rmse_direct, fig_rmse_sequential, nrow = 1, labels = c("B","", "C"), rel_widths = c(0.05,1.4,1.5))

fig3 <- plot_grid(fig_rmse_binary, fig3bc, nrow = 1, rel_widths = c(0.8,2.9), labels = c("A", ""))

ggsave("../Supplement/RMSE.jpg", fig3, width = 15, height = 4)
...
get_FOI <- rbind(data_paramsB, data_paramsB_null) %>% left_join(traj_props, by = c("strain", "Type", "Group")) %>%
  ...
  scale_color_manual("sHCoV", values = mycolors) + 
  scale_y_continuous("Seroconversion rate (1/years)", expand = c(0,0)) +
