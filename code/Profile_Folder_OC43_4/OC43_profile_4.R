...
profile_variation_4 <- function(i){
  ...
  dt_profile <- read_csv("data_in/return_fits.csv") %>%
    filter(strain == "OC43_HE", rho == "rho_95", boundaries == "Bounded", waning == "Laddered", exposure_hypothesis == "Variation") %>%
  ...
  data <- read_csv("data_in/data_all.csv") %>% filter(strain == "OC43_HE") %>% arrange(Age_con, Type, Group)
