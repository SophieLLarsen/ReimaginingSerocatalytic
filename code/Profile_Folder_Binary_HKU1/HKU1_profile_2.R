...
profile_2 <- function(i){
  ...
  dt_profile <- read_csv("data_in/return_fits.csv") %>%
    filter(strain == "HKU1_S1", rho == "rho_95", boundaries == "Bounded", waning == "Direct", exposure_hypothesis == "Binary") %>%
  ...
  data <- read_csv("data_in/data_all2.csv") %>% filter(strain == "HKU1_S1") %>% arrange(Age_con, Type, Group)
