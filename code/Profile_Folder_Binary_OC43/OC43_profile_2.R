...
profile_2 <- function(i){
  ...
  dt_profile <- read_csv("data_in/return_fits.csv") %>%
    filter(strain == "OC43_HE", rho == "rho_95", boundaries == "Bounded", waning == "Direct", exposure_hypothesis == "Binary") %>%
  ...
  data <- read_csv("data_in/data_all2.csv") %>% filter(strain == "OC43_HE") %>% arrange(Age_con, Type, Group)
