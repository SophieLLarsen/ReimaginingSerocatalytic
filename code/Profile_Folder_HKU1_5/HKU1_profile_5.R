...
#### Profile function ####
profile_variation_5 <- function(i){
  
  dt_profile <- read_csv("data_in/return_fits.csv") %>%
    filter(strain == "HKU1_S1", rho == "rho_95", boundaries == "Bounded", waning == "Laddered", exposure_hypothesis == "Variation") %>%
    select(paras, Estimate, loglik) %>% 
    mutate(upper = unlist(unname(c(list(rho_m = 365,
                                        l54 = 12, l43 = 12, l32 = 12, l21 = 12, l10 = 12,
                                        b0 = 8000, b1 = 8000, b2 = 8000, b3 = 8000,  sigma = 5)))),
           lower = unlist(unname(c(list(rho_m = 0,
                                        l54 = 1/80, l43 = 1/80, l32 = 1/80, l21 = 1/80, l10 = 1/80,
                                        b0 = 1, b1 = 1, b2 = 1, b3 = 1, sigma = 0.001))))
    )
  
  
  data <- read_csv("data_in/data_all.csv") %>% filter(strain == "HKU1_S1") %>% arrange(Age_con, Type, Group)
  ...
  start2 = list(
    ...
    l54 = dt_profile$Estimate[2], 
    l43 = dt_profile$Estimate[3], 
    l32 = dt_profile$Estimate[4], 
    l21 = dt_profile$Estimate[5], 
    l10 = dt_profile$Estimate[6], 
    b0 = dt_profile$Estimate[7], 
    b1 = dt_profile$Estimate[8], 
    b2 = dt_profile$Estimate[9], 
    b3 = dt_profile$Estimate[10], 
    sigma = dt_profile$Estimate[11]
  )
  ...
  if (round(dt_profile$Estimate[i],4) == round(dt_profile$upper[i],4)){
    ...
    while (precision > 10e-5) {
      ...
      if (parm_test_right >= upper_limit_var){
        ...
        m1_prof = mle2(minuslogl = loglik5_variation,
                       start = startB,
                       data = data,
